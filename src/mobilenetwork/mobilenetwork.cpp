	...
	: QObject(parent)
{
	...
	if (!actConReply.isValid()) {
		...
		if (!conReply.isValid()) {
			...
			m_activeConnection = qdbus_cast<QDBusObjectPath>(
	...
	QDBusConnection::systemBus().connect(
		"org.freedesktop.NetworkManager", m_path.path(),
...
void MobileNetwork::setMobileDataEnabled(bool mobileDataEnabled)
{
	if (!mobileDataEnabled) {
		QDBusInterface("org.freedesktop.NetworkManager", m_path.path(),
	...
	if (!connsReply.isValid()) {
		...
		foreach(QVariant connection, m_availableConnections) {
			if (connection.toMap().value("path").toString() ==
			    path) {
				syncOfonoContext(connection.toMap()
							 .value("name")
							 .toString(),
						 connection.toMap()
							 .value("apn")
...
void MobileNetwork::onDevicePropertiesChanged(QString iface,
...
void MobileNetwork::onPropertiesChanged(QString iface,
					QMap<QString, QVariant> updated,
					QStringList invalidated)
{
	Q_UNUSED(invalidated);
...
QMap<QString, QMap<QString, QVariant> >
MobileNetwork::getConnectionSettings(QDBusObjectPath path)
{
...
void MobileNetwork::setOfonoContextProperty(QString path, QString property,
					    QVariant value)
{
